...
- Generalise better to new situations
...
An LLM will state a fabricated statistic, an invented citation, or an incorrect legal requirement with exactly the same tone and certainty as a verified fact. There is no built-in signal that distinguishes accurate output from hallucination. If the information matters — and in business education it usually does — checking it is part of your workflow, not an optional extra.
:::

**The key insight:** LLMs interpolate, they do not retrieve. When you ask for a fact, the model is not looking it up. It is predicting what a plausible answer would sound like. A convincing answer and a correct answer are produced by the same process, and the model cannot tell you which one you are getting. This is why every verification habit in this book exists.
...
## A Useful Mental Model: Average vs Precise, Small vs Large

There is a simple way to predict when an LLM will serve you well and when it will let you down. Think along two axes.

**Average versus precise.** LLMs are excellent at producing convincing averages. Ask for a marketing email and you get something that reads like a competent marketing email, because the model has absorbed thousands of them and can synthesise a plausible representative. But ask for something precise, a specific legal precedent, a particular statistic from a named study, an exact API parameter, and the same mechanism becomes a liability. The model does not look things up. It predicts what a plausible answer would look like, which is how you end up with confident references to sources that do not exist.

**Small versus large.** LLMs handle small, bounded tasks well. A focused prompt with clear constraints and one shape for the answer plays to the model's strengths. But as tasks grow larger, with more interdependent decisions and conflicting constraints, the output starts to look coherent on the surface while falling apart in the details.

These two axes interact. Being good at the average means the model will confidently produce plausible-looking large outputs that fail on the precise details. The convincing average scales badly with complexity.

This gives you a practical decision framework:

| | Small | Large |
|---|---|---|
| **Average** | **Sweet spot.** Drafts, summaries, brainstorming. | **Plausible but brittle.** Looks right, falls apart on inspection. |
| **Precise** | **Workable with verification.** Facts, code, citations. Check before using. | **Danger zone.** Confident structure, subtly wrong. |

Before using AI on any task, ask: **How precise does this need to be?** and **How big is this?** The answers tell you how much human oversight the task requires. For a deeper treatment of this framework and how it connects to staying in the conversation loop, see *Conversation, Not Delegation*.
